...
use crate::key::{PrivateKey, PublicKey, PK_SEED_BYTES, SALT_LEN_BYTES, SK_SEED_BYTES};
use aes::{
    cipher::{generic_array::GenericArray, BlockEncrypt, KeyInit},
    Aes128,
};
use ndarray::{stack, Array1, Array2, Array3, ArrayView2};
use num_traits::Zero;
use sha3::{
...
pub fn aes_hash(seed: &[u8; 16], out: usize) -> Vec<F256Point> {
    ...
    while bytes_remaining > 0 {
        ...
        ctr += 1;
        bytes_remaining = if bytes_remaining >= 16 {
            bytes_remaining - 16
        } else {
            0
        };
...
mod tests {
    ...
    fn test_key_gen() {
        ...
        let (_esk, epk) = key_gen(seed_sk, seed_pk, 112, 44).unwrap();
        for i in 0..44 {
